...
pub fn one_hot_encoding<T: Float>(data: Vec<usize>, size:usize, _: T) -> Vec<Vector<T>> {
    assert!(*data.iter().max().unwrap_or(&0usize) < size, "!!!Size of the vector must be greater then max number:\
     Vector size: {size}, Max Element: {}!!!", data.iter().max().unwrap_or(&0usize));
...
pub fn one_hot_decoding<T: Float>(data: Vec<Vector<T>>) -> Vec<usize> {
    data
        .iter()
        .map(|x| x.data.iter().position(|&x| x == T::one()).unwrap_or_default())
        .collect()
}
